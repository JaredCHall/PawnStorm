...
const testPosition = (fen: string, depth: number, expectedNodes: number): void => {
    Deno.test(`it finds ${expectedNodes} nodes in position: "${fen}" position at depth ${depth}`, async () => {
        assertEquals(
            await new PerftRunner(fen).runAsync(depth, 4),
            expectedNodes
        )
    })
}
...
for(const i in tests){
    ...
    const fen = parts[1]
    const depth = parseInt(depthParts[1])
    const expectedNodes = parseInt(depthParts[2])

    testPosition(fen, depth, expectedNodes)
}
